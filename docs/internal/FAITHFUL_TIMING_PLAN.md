...
`integrate/upstream-2026-09-15` branch. Reconciled CFG/discovery and static
overlay tooling with authenticated source observations, guest-clock audio,
Native rendering, and the updated launcher. Overlay ABI/codegen are 26/19;
the combined strict savestate format is version 10 with a 108-byte header,
distinct Native/provenance/mod-memory sections, and enhancement-memory rollback.
Regenerated both BIOS backends and game/overlay C through the generators.

The first executable replay caught a real integration failure at VBlank 2024:
`Native packet producer rejected a DMA word`. GDB showed a structural mismatch
between the eager whole-list reservation and the packet actually read by DMA
(`last_consume_status=2`). Upstream's event-driven linked-list walker allows
guest writes to affect links/payloads before their consumption; retaining the
old synchronous walk's eager reservation was therefore invalid. The asynchronous
path now uses the existing packet-completion preflight on the words actually
consumed. It carries the preceding node through the serializable DMA walker and
publishes incoming-link provenance at the read boundary. Source authentication
and packet validation remain active; linked-list DMA is not drained eagerly.
Also fixed a merged GL fragment shader's missing `u_shift` declaration.

Validation: the complete Xenogears Debug runtime builds and links. The original
4,862-VBlank replay completes with exit 0, `runtime_status=PASS`, Native work
health `healthy`, stable snapshots, and no fatal halt. Sampled CD underflow and
host underrun counts remain zero. Local TCP/PCM receipts:
`/home/pc/opencode-tmp/opencode/xg-audio-20260915-upstream-merge-final/`.
The legacy authorization receipt still says `status=FAIL`, as it did before
the merge; this does not establish pixel equivalence. No screenshots were used.
Earlier focused checks passed seven runtime device/mod/savestate CTests,
41 overlay-tool pytest cases, and eight project-side static-dispatch tests;
the standalone UI integration passed 20 CTests. Per user direction, obsolete
Native-renderer tests from the prior rework are not integration gates.
