...
typedef struct {
    GuestRenderProducerHandle handle;
    /* Normalized word-aligned physical RAM address. */
...
typedef struct {
    ...
    /* The first fallback in the current scene, or NONE for a clean scene. */
    GuestRenderFallbackReason fallback_reason;
    /* The most recent fallback across all scenes, retained at scene boundaries. */
    GuestRenderFallbackReason last_fallback_reason;
    bool state_open;
    bool producer_open;
    size_t slot_count;
    size_t binding_count;
    /* Lifetime count of first-fallback transitions, at most one per scene. */
