...
/* v17: ws_mask_or forwarder for configured trim-mask merge cull sites.
 *      Appended last; NULL preserves the vanilla merge. */
#define PSX_OVERLAY_ABI_VERSION 17
 ...
 *   7: + configured trim-mask merge (mask_or) cull emit. */
#define PSX_OVERLAY_CODEGEN_VER 7
...
typedef struct {
    ...
    /* Trim-mask merge gate (ABI v17). Runtime-state dependent, so overlay
     * code forwards to the host; NULL preserves the vanilla merge. */
    uint32_t (*ws_mask_or)(uint32_t mask);
} OverlayCallbacks;
