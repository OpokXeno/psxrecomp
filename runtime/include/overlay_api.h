 ...
 *      loaded value. */
/* v19: check_interrupts_at returns whether exception handling redirected the
 *      compiled edge. Generated code returns to the dispatcher on redirect. */
/* v21: certified Xenogears resident field frame-step helper. */
/* v22: frame-step helper receives the resident authored-frame token.
...
/* v26: combine Native producer hooks with upstream signed screen bounds and
 * mod-entry forwarding. Previous fork/upstream layouts are not interchangeable. */
 ...
 *  14: authenticated native-render entry/capture/return lifecycle hooks.
 *  15: direct-JAL RETURN is emitted at proven continuation arrival.
 ...
 *  19: combined ABI v26 screen-bound, mod-entry and Native forwarders. */
#define PSX_OVERLAY_CODEGEN_VER 19
...
typedef struct {
    ...
    int (*check_interrupts_at)(CPUState *cpu, uint32_t resume_pc);
    ...
    /* GTE coprocessor 2 execution with its exact guest instruction PC. */
