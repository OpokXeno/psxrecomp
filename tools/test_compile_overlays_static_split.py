...
class StaticDispatchApiTests(unittest.TestCase):
    ...
    def test_empty_dispatcher_exports_can_dispatch_api(self):
        dispatch = compile_overlays.generate_overlay_dispatch([], self.identity)
    ...
    def test_can_dispatch_uses_sha_lookup_and_does_not_invoke_cpu(self):
        ...
        dispatch = compile_overlays.generate_overlay_dispatch(variants, self.identity)
        with tempfile.TemporaryDirectory() as tmp:
            ...
            Path(src).write_text(textwrap.dedent(f'''
                ...
                static uint8_t resident_sha256[32];
                ...
                {{
                    ...
                    (void)count;
                    match_checks++;
                    return memcmp(expected_sha256, resident_sha256, 32) == 0;
                }}
                ...
                {{
                    ...
                    memset(resident_sha256, 0x22, sizeof(resident_sha256));
                    ...
                    memset(resident_sha256, 0x33, sizeof(resident_sha256));
            ...
            '''), encoding='utf-8')
            runtime = Path(compile_overlays.__file__).resolve().parents[1] / 'runtime'
            subprocess.run([cc, src, str(runtime / 'src/game_identity.c'),
                            '-I', tmp, '-I', str(runtime / 'include'),
                            '-DPSX_GAME_EXTRA_IDENTITY_SHA256="' + '01' * 32 + '"',
                            '-DPSX_GAME_MANIFEST_DIGEST_SHA256="' + '02' * 32 + '"',
                            '-std=c99', '-Wall', '-Werror',
                            '-o', exe], check=True)
...
class StaticWorkerContractTests(unittest.TestCase):
    ...
    def test_data_only_capture_reports_skip_with_captured_log(self):
        ...
        self.assertIn('SKIP: no shared walk-root seeds', res['log'])
