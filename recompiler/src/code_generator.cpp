...
std::string CodeGenerator::translate_instruction(uint32_t addr, uint32_t instr) {
    ...
    /* Neither a source-commit hook nor a disassembly comment may be appended
     * to a preprocessor directive (including mfhi/mflo to $zero). */
    if (PSXRecomp::emission_ends_on_preprocessor_directive(code))
        code += "\n" + config_.indent;
