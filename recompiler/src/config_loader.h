...
struct RuntimeConfig {
    ...
    // Host presentation AA: off, FXAA, SMAA, TAA, MSAA, SSAA.
    int                   video_antialiasing = 1;
    // Independent quality multiplier. SSAA/MSAA use N spatial samples;
    // FXAA/SMAA search farther; TAA accumulates N jitter phases.
    // Values: 1, 2, 4, 8, 16.
    int                   video_antialiasing_factor = 4;
    ...
    int                   video_vsync             = 1;
    // Presentation rate. 30 preserves original PSX cadence; 60 enables the
    // Native interpolation path. Other rates are intentionally not exposed.
    int                   video_fps = 30;
...
struct GameConfig {
    ...
    // Extra per-side actor/render/terrain participation beyond the visible edge.
