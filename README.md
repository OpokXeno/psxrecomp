...
| A file reachable only through a game-specific hidden index | Authenticated `[[indexed_file]]` replacement (format 6), with conditional composition metadata in format 7; format 8 also scopes executable patches by disc and feature state; requires a statically registered game handler |
